...
### Important notes - need to get remote orderer TLS CA and PEER TLS CA (if both using different TLS)
...
export REMOTE_PEER_ADDRESS=<REMOTE_URL>:<REMOTE_PORT>
export REMOTE_PEER_CA=${PWD}/temp/ca.crt
...
if [ $2 = "commit" ]; then
  ./bin/peer lifecycle chaincode commit -o ${ORDERER_HOST}:${ORDERER_PORT} --ordererTLSHostnameOverride ${ORDERER_HOST} --tls --cafile ${ORDERER_CA} --channelID ${CHANNEL_NAME} --name ${CC_NAME} --peerAddresses ${CORE_PEER_ADDRESS} --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE} --peerAddresses ${REMOTE_PEER_ADDRESS} --tlsRootCertFiles ${REMOTE_PEER_CA} --version ${CC_VERSION} --sequence ${CC_SEQUENCE} --init-required
fi
...
if [ $2 = "invoke-init" ]; then
  ./bin/peer chaincode invoke -o ${ORDERER_HOST}:${ORDERER_PORT} --ordererTLSHostnameOverride ${ORDERER_HOST} --tls --cafile ${ORDERER_CA} --channelID ${CHANNEL_NAME} --name ${CC_NAME} --peerAddresses ${CORE_PEER_ADDRESS} --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE} --peerAddresses ${REMOTE_PEER_ADDRESS} --tlsRootCertFiles ${REMOTE_PEER_CA} --isInit -c '{"function":"Initialize","Args":["Wrapped ZTX", "wZTX", "6"]}'
fi

if [ $2 = "invoke-mint" ]; then
  ./bin/peer chaincode invoke -o ${ORDERER_HOST}:${ORDERER_PORT} --ordererTLSHostnameOverride ${ORDERER_HOST} --tls --cafile ${ORDERER_CA} --channelID ${CHANNEL_NAME} --name ${CC_NAME} --peerAddresses ${CORE_PEER_ADDRESS} --tlsRootCertFiles ${CORE_PEER_TLS_ROOTCERT_FILE} --peerAddresses ${REMOTE_PEER_ADDRESS} --tlsRootCertFiles ${REMOTE_PEER_CA} -c '{"function":"Mint","Args":["5000"]}'
fi
